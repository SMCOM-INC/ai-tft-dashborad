...
const useSurveyState = ({ state, finishFlag, openTime }) => {
  // ============================================
  // 매개변수
  // ============================================
  ...
  const checkAuth = computed(() => {
    ...
    if (stateValue.value === SURVEY_STATE.PENDING) {
    ...
    if (stateValue.value === SURVEY_STATE.CLOSE) {
  ...
  const isPendingState = computed(() => {
    return stateValue.value === SURVEY_STATE.PENDING;
  });
