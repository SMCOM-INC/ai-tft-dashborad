<script setup>
  ...
  import useGetSurveyDetailDefaultEtcList from '@/lib/queries/survey/useGetSurveyDetailDefaultEtcList.js';
  ...
  const { surveyDetailDefaultEtcList, isSurveyDetailDefaultEtcListLoading } =
    useGetSurveyDetailDefaultEtcList({ questionUuid: props.questionUuid });
...
<template>
  <ModalBaseNew>
    ...
    >
      ...
      >
        <div class="pretendard-18SemiBold">기타 응답 전체보기</div>
        <ButtonBase
          type="button"
          color="secondary"
          size="md"
      ...
      >
        <template v-if="isSurveyDetailDefaultEtcListLoading">
        ...
        <template v-else>
          <span class="p-2 text-brand-default-text-brand pretendard-14SemiBold">
            응답 {{ surveyDetailDefaultEtcList?.length || 0 }}개
          </span>
          <ul class="flex w-full flex-col gap-2">
            <li
              v-for="(response, responseIndex) in surveyDetailDefaultEtcList"
              :key="responseIndex"
          ...
          <div
            v-if="
              ...
              surveyDetailDefaultEtcList.length === 0
            "
