<script setup>
  ...
  import useGetSurveyFormSampleExcel from '@/lib/queries/survey/useGetSurveyFormSampleExcel.js';
  import usePatchSurveyFormExcel from '@/lib/queries/survey/usePatchSurveyFormExcel.js';
  import decodeUrl from '@/lib/utils/decodeUrl.js';
  import formatFileSize from '@/lib/utils/formatFileSize.js';

  const emits = defineEmits(['close']);

  const { isSurveyFormSampleExcelLoading, refetchSurveyFormSampleExcel } =
    useGetSurveyFormSampleExcel();
  ...
  const {
    ...
    surveyFormExcelProgressPercent,
    isPatchSurveyFormExcelError,
    patchSurveyFormExcelError,
  } = usePatchSurveyFormExcel();
  ...
  const {
    ...
    mutationAsyncFunction: patchSurveyFormExcelMutationAsync,
    mutationData: patchSurveyFormExcelData,
    isMutationPending: isPatchSurveyFormExcelPending,
    isMutationSuccess: isPatchSurveyFormExcelSuccess,
    isMutationError: isPatchSurveyFormExcelError,
    mutationError: patchSurveyFormExcelError,
  });
  ...
  <ModalBaseNew>
    <div class="w-[512px] space-y-6 p-6">
      ...
      <div class="flex justify-between gap-2">
        ...
        <button
          type="button"
          :disabled="isPatchSurveyFormExcelPending"
          @click="closeModal"
        >
        ...
        <template v-if="isBefore">
          <div class="flex justify-end">
            <button
              ...
              @click="refetchSurveyFormSampleExcel"
            >
              샘플 엑셀파일 다운로드
              <SpinnerCircle
                v-if="isSurveyFormSampleExcelLoading"
                color="blue"
                class="flex"
              />
            </button>
          ...
          <div v-if="isUploading" class="flex flex-col items-end gap-1">
            ...
            >
              <div
                ...
                :style="{ width: `${surveyFormExcelProgressPercent}%` }"
              />
            </div>
          </div>
          <!-- 에러 문구 -->
