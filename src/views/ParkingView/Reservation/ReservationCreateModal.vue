<script setup>
  import CloseIcon from '@assets/icons/icon-close-darkGray.svg';
  import ButtonBase from '@components/common/ButtonBase.vue';
  import LabelBase from '@components/common/LabelBase.vue';
  import ModalBaseNew from '@components/common/ModalBaseNew.vue';
  import ModalFieldCarNum from '@components/common/ModalFieldCarNum.vue';
  import ModalFieldDongHo from '@components/common/ModalFieldDongHo.vue';
  import ModalFieldPhone from '@components/common/ModalFieldPhone.vue';
  import ModalFieldVisitPurpose from '@components/common/ModalFieldVisitPurpose.vue';
  import SpinnerWhiteView from '@components/common/SpinnerWhiteView.vue';
  import TextError from '@components/common/TextError.vue';
  import { toTypedSchema } from '@vee-validate/zod';
  import VueDatePicker from '@vuepic/vue-datepicker';
  import { useForm } from 'vee-validate';
  import { computed, watch } from 'vue';

  import useWallPadContent from '@/lib/composables/parking/useWallPadContent.js';
  import usePostReservation from '@/lib/queries/parkingReservation/usePostReservation.js';
  import swalConfirmModal from '@/lib/swalModal/swalConfirmModal.js';
  import { formatDateTimeObject } from '@/lib/utils/formatDate.js';
  import {
    visitReservationFormSchema,
    visitReservationFormSchemaWithWallPadAlarm,
  } from '@/schemas/parking.js';

  const emits = defineEmits(['close']);

  const { hasWallPadUI } = useWallPadContent();

  const afterSevenDays = computed(() => {
    const date = new Date();
    date.setDate(date.getDate() + 6);
    return date;
  });

  const schema = computed(() => {
    if (hasWallPadUI) {
      return toTypedSchema(visitReservationFormSchemaWithWallPadAlarm);
    }
    return toTypedSchema(visitReservationFormSchema);
  });

  const { errors, defineField, handleSubmit } = useForm({
    validationSchema: schema,
  });

  const [inoutParkingScheduledDate] = defineField('inoutParkingScheduledDate');
  const [parkingWallPadAlarm] = defineField('parkingWallPadAlarm');

  const {
    postReservationMutation,
    isPostReservationPending,
    isPostReservationSuccess,
  } = usePostReservation();

  const onSubmit = handleSubmit((submitValues) => {
    swalConfirmModal({
      title: '방문예약 등록',
      text: '방문예약을 등록하시겠습니까?',
      callback: () => {
        postReservationMutation({
          carNum: submitValues.carNum,
          dong: submitValues.dong,
          ho: submitValues.ho,
          inParkingScheduledDate: submitValues.inoutParkingScheduledDate
            ? formatDateTimeObject(
                submitValues.inoutParkingScheduledDate[0],
                'start',
              )
            : null,
          outParkingScheduledDate: submitValues.inoutParkingScheduledDate
            ? formatDateTimeObject(
                submitValues.inoutParkingScheduledDate[1],
                'end',
              )
            : null,
          phone: submitValues.phone,
          visitPurposeUuid: submitValues.visitPurposeUuid,
          parkingWallPadAlarm: submitValues.parkingWallPadAlarm,
        });
      },
    });
  });

  const closeModal = () => {
    emits('close');
  };

  watch(
    () => isPostReservationSuccess.value,
    () => {
      closeModal();
    },
  );
</script>

<template>
  <ModalBaseNew>
    <div
      class="max-h-[80vh] w-full max-w-[512px] overflow-y-auto rounded-lg bg-white p-6"
    >
      <header class="mb-4 flex items-center justify-between">
        <h1 class="select-none text-lg font-medium">세대 방문 차량 예약하기</h1>
        <button type="button" class="w-fit" @click="closeModal">
          <CloseIcon class="h-5" aria-hidden="true" />
        </button>
      </header>
      <form id="createReservationForm" @submit="onSubmit">
        <ul class="flex flex-col gap-4">
          <!-- 차량번호 -->
          <ModalFieldCarNum :errors="errors" />
          <!-- 동호수 -->
          <ModalFieldDongHo :errors="errors" />
          <!-- 연락처 -->
          <ModalFieldPhone id="phone" :errors="errors" label-text="연락처" />
          <!-- 입출차예정일 -->
          <li class="flex flex-col gap-2">
            <LabelBase
              label-for="inoutCarDate"
              label-text="입출차 예정일"
              asterisk
            />
            <VueDatePicker
              v-model="inoutParkingScheduledDate"
              range
              locale="ko"
              :enable-time-picker="false"
              auto-apply
              placeholder="YYYY.MM.DD ~ YYYY.MM.DD"
              format="yyyy.MM.dd"
              class="custom-date-picker"
              :min-date="new Date()"
              :max-date="afterSevenDays"
            />
            <p class="text-xs leading-5 text-muted-foreground-100">
              방문기간 예약 설정은 최대 7일입니다.
            </p>
          </li>
          <!-- 방문목적 -->
          <ModalFieldVisitPurpose :errors="errors" />
          <!-- 입출차 시 월패드 알림 -->
          <li v-if="hasWallPadUI" class="flex flex-col gap-2">
            <LabelBase
              label-for="parkingWallPadAlarm"
              label-text="입출차 시 월패드 알림"
              asterisk
            />
            <div>
              <ul class="flex w-full gap-2">
                <li class="flex w-full gap-2 rounded-md border p-3">
                  <input
                    id="parkingWallPadAlarm-yes"
                    v-model="parkingWallPadAlarm"
                    type="radio"
                    name="parkingWallPadAlarm"
                    :value="true"
                  />
                  <LabelBase
                    label-for="parkingWallPadAlarm-yes"
                    label-text="예"
                  />
                </li>
                <li class="flex w-full gap-2 rounded-md border p-3">
                  <input
                    id="parkingWallPadAlarm-no"
                    v-model="parkingWallPadAlarm"
                    type="radio"
                    name="parkingWallPadAlarm"
                    :value="false"
                  />
                  <LabelBase
                    label-for="parkingWallPadAlarm-no"
                    label-text="아니오"
                  />
                </li>
              </ul>
              <TextError v-if="errors.parkingWallPadAlarm">{{
                errors.parkingWallPadAlarm
              }}</TextError>
            </div>
          </li>
        </ul>
        <div class="pt-6">
          <ButtonBase
            form="createReservationForm"
            type="submit"
            color="primary"
            custom-class="w-full"
          >
            <div
              v-if="isPostReservationPending"
              class="flex items-center justify-center gap-2"
            >
              <span>예약중..</span>
              <SpinnerWhiteView />
            </div>
            <div class="w-full">예약하기</div>
          </ButtonBase>
        </div>
      </form>
    </div>
  </ModalBaseNew>
</template>

<style scoped>
  .custom-date-picker {
    --dp-font-family: 'Pretendard', -apple-system, 'Segoe UI', roboto, oxygen,
      ubuntu, cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;
    --dp-font-size: 15px !important;
    --dp-font-weight: 600 !important;
  }
  .custom-date-picker :deep(.dp__input) {
    line-height: calc(var(--dp-font-size) * 1.8) !important;
  }
</style>
