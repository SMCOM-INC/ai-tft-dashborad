<script setup>
  ...
  const { deleteNotOutHistoryMutationAsync, isDeleteNotOutHistoryPending } =
    useDeleteNotOutHistory(outHistoryResendRef);
  ...
  const renderFieldValue = (key) => {
    ...
    if (key === 'phone') {
      result = formatContact(value);
    }
    if (key === 'inParkingTime' || key === 'outParkingTime') {
      result = value ? formatDate(value).full() : null;
    }
    if (key === 'parkingMinute') {
      result = value ? formatMinutes(value) : null;
    }
    if (key === 'registType') {
      result = findRegistType(value);
    }
    if (key === 'carType') {
      result = findCarType(value);
    }
    if (!result) {
      result = value;
    }
...
<template>
  ...
  >
    ...
    <div class="mb-6 flex items-start justify-between">
      <h2 class="pretendard-h3">미출차 상세</h2>
    ...
    <ul v-if="inOutHistoryDetail">
      <div
        v-if="inOutHistoryDetail"
        class="flex flex-col gap-6 text-sm font-normal"
      >
