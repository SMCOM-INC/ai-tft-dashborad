<script setup>
  ...
  import OutNotHistoryDeleteModal from '@views/ParkingView/NotOutHistory/NotOutHistoryDeleteModal.vue';
  import NotOutHistoryProgressbarModal from '@views/ParkingView/NotOutHistory/NotOutHistoryProgressbarModal.vue';
  ...
  import useDeleteNotOutHistory from '@/lib/queries/parkingNotoutHistory/useDeleteNotOutHistory.js';
  ...
  // 모달 상태
  const outHistoryResendRef = ref(null);
  ...
  const { deleteNotOutHistoryMutationAsync, isDeleteNotOutHistoryPending } =
    useDeleteNotOutHistory();
  ...
  const deleteItem = async (item, uuid) => {
    try {
      await deleteNotOutHistoryMutationAsync({
        uuid,
        skipSuccess: true,
        skipError: true,
      });
  ...
  const handleSingleDeletion = async (uuid) => {
    try {
      await deleteNotOutHistoryMutationAsync({ uuid });
      closeDeletionModal();
      progressState.value = 'COMPLETED';
    } catch (error) {
      closeDeletionModal();
  ...
  // 메인 삭제 함수
  const deleteNotOut = async () => {
    if (props.selectedRows.length === 0) {
      alert('삭제할 미출차를 선택해주세요.');
      closeDeletionModal();
      return;
    }
...
<template>
  <!-- 입출차 동기화 모달 -->
  <ModalInOutHistoryResend ref="outHistoryResendRef" hide-button />

  <!-- 미출차 삭제 확인 모달 -->
  <OutNotHistoryDeleteModal
    v-if="isDeleteModalOpen"
    :is-loading="isDeleteNotOutHistoryPending"
    @close="closeDeletionModal"
    @delete="deleteNotOut"
  />

  <!-- 프로그레스 모달 -->
  <NotOutHistoryProgressbarModal
    v-if="isProgressModalOpen"
    :state="progressState"
    :total-count="progressTotal"
    :current-count="progressCurrent"
    :success-count="progressSuccess"
    :failed-count="progressFailed"
