<script setup>
  ...
  import VoteCreateFormExcelPreviewModal from '@views/VoteView/Form/VoteCreateFormExcelPreviewModal.vue';
  import VoteCreateFormExcelUploadModal from '@views/VoteView/Form/VoteCreateFormExcelUploadModal.vue';
  ...
  const { setExcelInfo } = useVoteSubmitStore();
  ...
  watch(
    ...
    (newValue) => {
      ...
      excelFile.value = newValue.voterList;
    },
    { immediate: true },
  );
...
<template>
  <div>
    ...
    <VoteCreateFormExcelUploadModal
      v-if="modalType === 'fileUpload'"
      @close="closeModal"
    />
    <VoteCreateFormExcelPreviewModal
      v-if="excelFile?.length > 0 && modalType === 'filePreview'"
      :voter-list="excelFile"
      @close="closeModal"
    />
  </div>
</template>
