<script setup>
  ...
  import VoteCreateFormDefault from '@views/VoteView/Form/VoteCreateFormDefault.vue';
  import VoteCreateFormExcel from '@views/VoteView/Form/VoteCreateFormExcel.vue';
  import VoteCreateFormQuestion from '@views/VoteView/Form/VoteCreateFormQuestion.vue';
  import VoteCreateSubmitBar from '@views/VoteView/Form/VoteCreateSubmitBar.vue';
  import { computed, ref, shallowRef, watch } from 'vue';

  import useNavigate from '@/lib/composables/common/useNavigate.js';
  import useUnsavedChangesGuard from '@/lib/composables/common/useUnsavedChangesGuard.js';
  import useGetVoteFormDetail from '@/lib/queries/vote/useGetVoteFormDetail.js';

  const { getCurrentRoutePath } = useNavigate();

  const { voteFormDetail, isVoteFormDetailLoading, isVoteFormDetailError } =
    useGetVoteFormDetail();
  ...
  const sections = ref([
    {
      ...
      title: 'STEP 1 : 기본 정보',
      isOpen: true,
      component: shallowRef(VoteCreateFormDefault),
    },
    {
      key: 'participants',
      title: 'STEP 2 : 참여자 업로드',
      isOpen: true,
      component: shallowRef(VoteCreateFormExcel),
    },
    {
      key: 'questions',
      title: 'STEP 3 : 질문 항목',
      isOpen: true,
      component: shallowRef(VoteCreateFormQuestion),
    },
  ]);
  ...
  watch(isVoteFormDetailError, (newValue) => {
    if (newValue) {
      hasGuard.value = false;
    }
  });
...
<template>
  <div class="pb-[130px]">
    <div class="flex justify-between">
      <PageTitleBase
        :title="isCreatePage ? '투표 생성' : '투표 수정'"
        has-back-button
      />
    </div>
    <div class="mx-auto max-w-[924px]">
      <section v-for="section in sections" :key="section.key">
        ...
        <component
          ...
          v-bind="{
            detailInfo: voteFormDetail,
            isDetailInfoLoading: isVoteFormDetailLoading,
          }"
