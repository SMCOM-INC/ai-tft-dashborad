<script setup>
  ...
  const { value: openVoteDate } = useField('openVoteDate');
  const { value: openVoteHours } = useField('openVoteHours', undefined, {
    initialValue: initialTime.hours,
  });
  const { value: openVoteMinutes } = useField('openVoteMinutes', undefined, {
    initialValue: initialTime.minutes,
  });

  const { value: closeVoteDate } = useField('closeVoteDate');
  const { value: closeVoteHours } = useField('closeVoteHours', undefined, {
    initialValue: '00',
  });
  const { value: closeVoteMinutes } = useField('closeVoteMinutes', undefined, {
    initialValue: '00',
  });
</script>
...
<template>
  <div class="flex gap-4">
    ...
    <div class="flex w-[364px] gap-1">
      <InputDatePicker
        id="openVoteDate"
        v-model="openVoteDate"
        local="ko"
        format="yyyy-MM-dd"
        label="시작일시"
        :min-date="new Date()"
        hint="시작일시를 선택해주세요."
        :error="errors.openVoteDate"
        class="w-[274px]"
      />
      <select
        v-model="openVoteHours"
    ...
    <div class="flex w-[364px] gap-1">
      <InputDatePicker
        id="closeVoteDate"
        v-model="closeVoteDate"
        local="ko"
        format="yyyy-MM-dd"
        label="종료일시"
        :min-date="new Date()"
        hint="종료일시를 선택해주세요."
        :error="errors.closeVoteDate"
      />
      <select
        v-model="closeVoteHours"
        class="mt-8 h-[38px] w-28 rounded-[4px] border border-defaults-primary-border-primary bg-[url('@assets/icons/icon-chevron-down-black.svg')] bg-[right_12px_center] bg-no-repeat px-3 py-2 text-sm"
        required
      >
        <option
