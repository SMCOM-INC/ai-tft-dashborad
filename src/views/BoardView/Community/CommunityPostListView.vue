<script setup>
  ...
  import {
    BOARD_COMMUNITY_POSTS_TABLE_COLUMNS_LIST,
  ...
  import useGetCommunityList from '@/lib/queries/boardCommunity/useGetCommunityList.js';
  ...
  const clickRow = (row) => {
    navigateTo(`/board/community/posts/detail/${row.uuid}`);
  };
...
<template>
  <SearchBar
    ...
    has-reset
    :is-search-loading="isCommunityPostListLoading"
    class="mb-7"
  >
    <CommunityFilters />
  </SearchBar>
  <TableView
    :column-data="BOARD_COMMUNITY_POSTS_TABLE_COLUMNS_LIST"
    :page-data="communityPostList"
    show-count
    selectable
    :row-function="clickRow"
    :is-loading="isCommunityPostListLoading"
    :is-error="isCommunityPostListError"
