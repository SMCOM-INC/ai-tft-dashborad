<script setup>
  ...
  const { value: closeDate } = useField('closeDate', undefined, {
    initialValue: new Date(),
  });
  const { value: closeHours } = useField('closeHours', undefined, {
    initialValue: '00',
  });
  const { value: closeMinutes } = useField('closeMinutes', undefined, {
    initialValue: '00',
  });
  // 현재 날짜 정보
...
<template>
  <div class="space-y-2">
    <LabelBase label-for="closeDate" label-text="출차 일시" asterisk />
    <div class="flex gap-1">
      ...
      <select
        v-model="closeHours"
        class="h-[38px] w-[25%] rounded-[4px] border border-defaults-primary-border-primary bg-[url('@assets/icons/icon-chevron-down-black.svg')] bg-[right_12px_center] bg-no-repeat px-3 py-2 text-sm"
        required
      >
        <option
